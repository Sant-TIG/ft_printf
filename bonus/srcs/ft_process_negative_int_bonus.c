...
/*   Updated: 2022/02/11 09:29:11 by sperez-p         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	else if (flags->zero > 0)
	{
		if (flags->precision > 0)
			flags->len += ft_process_width(unbr_str, flags)
				+ ft_putchar('-') + ft_putstr(unbr_str);
		else
			flags->len += ft_putchar('-') + ft_process_width(unbr_str, flags)
				+ ft_putstr(unbr_str);
	}
	else
		flags->len += ft_process_width(unbr_str, flags)
...
void	ft_process_negative_int(char *unbr_str, t_bonus *flags)
{
	...
	else
	{
		...
		if (flags->width > 0)
			...
			flags->len += ft_putchar('-')
				+ ft_putstr(unbr_str) + ft_process_width(unbr_str, flags);
