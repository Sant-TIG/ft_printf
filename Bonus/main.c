...
int	ft_process_width(char *arg, char *dst, size_t len)
{
	size_t	i;
	int	nbr;
	char *str;
...
char *ft_get_precision(char *dst)
{
	char 	*str;
	size_t	i;

	i = -1;
	str = (char *)malloc(sizeof(char) * (ft_prclen(dst) + 1));
...
char *ft_process_precision(char *arg, char *dst, char id)
{
	size_t	i;
	char	*str;
	char	*prc;
...
int	ft_process_string(char *arg, char *dst, char id)
{
	int		len;
	size_t	i;
	...
	arg = ft_process_precision(arg, dst, id);
	if (dst[i] == '-')
	{
		i++;
		len = ft_putstr(arg) + ft_process_width(arg, &dst[i], ft_strlen(arg));
	}
	if (ft_isdigit(dst[i]))
	{
		len = ft_process_width(arg, &dst[i], ft_strlen(arg)) + ft_putstr(arg);
	}
	else
...
int	ft_write_specifier(va_list ap, char *dst, char id)
{
	int	len;
	...
	if (id == 's')
	{
		len = ft_process_string(va_arg(ap, char *), dst, id);
	}
	return (len);
}
...
char	ft_identify_specifier(char *str, int i)
{
	while(!ft_is_specifier(str[i]))
		i++;
	return (str[i]);
...
int	ft_printf(const char *format, ...)
{
	...
	i = 0;
	format_len = 0;
	dst = (char *)malloc(sizeof(char) * (ft_strlen((char *)format) + 1));
	va_start(ap, format);
	ft_strcpy(dst, format, ft_strlen((char *)format));
	while (dst[i])
	{
		if (dst[i] == '%' && dst[i + 1] != '%')
		{
			id = ft_identify_specifier(dst, i);
			format_len = ft_write_specifier(ap, &dst[i + 1], id) + i;
			i += ft_spclen(dst, i) + 1;
		}
		else
...
int main ()
{
	...
	int len1 = ft_printf("Esto es una cadena: %10.4s y esta es otra: %-7.5s.", "cadena", "cadena");
